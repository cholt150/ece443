...
{
static int LEDAStatus = 0;
CANTxMessageBuffer * message; /* Pointer to the CAN message buffer */

    PORTToggleBits(IOPORT_G, LED1);                 /* Toggle LED1 */
    PORTToggleBits(IOPORT_B, LEDA);                 /* Toggle LEDA */
    LEDAStatus = PORTReadBits(IOPORT_B, LEDA);;     /* Read LEDA status */
...
//      message->msgEID.EID	= 0x4004;   // Send message to CAN2.
        ...
        message->data[0] = LEDAStatus;  // This is the payload.
...
/* This function will send a message to CAN1 with EID 8004001. The data payload
 *size is 1 byte. The value of the LEDBStatus will be toggled and then sent as
 * the payload. */

    CANTxMessageBuffer * message;		/* Pointer to the CAN message buffer */
    PORTToggleBits(IOPORT_G, LED2);             /* Toggle LED2 */
    PORTToggleBits(IOPORT_B, LEDB);             /* Toggle LEDB */
    LEDBStatus = PORTReadBits(IOPORT_B, LEDB);  /* Read LEDB Status */
...
/* Form a Extended ID message. Start by clearing the buffer. */
        ...
        message->msgEID.RTR = 0;            /* Not an RTR message. */
        message->msgEID.DLC = 1;            /* Send one byte of data. */
        message->data[0]    = LEDBStatus;   /* This is the payload. */
