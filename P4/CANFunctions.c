 ...
 * DESCRIPTION:     This function initializes CAN1 for standard 11 bit ID. This
 ...
 *                  receive a specific message, in this case 
 *                  SID LED1_INDICATION_MSG by the value specified for
 *                  CAN_FILTER0.
 * PARAMETER1:      None
 * RETURN VALUE:    None
 * Notes:           None
 ...
 */
    ...
    CANConfigureFilter      (CAN1, CAN_FILTER0, LED1_INDICATION_MSG, CAN_SID);

    CANConfigureFilterMask  (CAN1, CAN_FILTER_MASK0, SID_FILTER_MASK, CAN_SID,
 ...
 * message with SID CAN2_MSG_ID. Note that filter 0 points to a
 * transmit channel (Channel 0). This is only possible because Channel 0 is
 * RTR enabled. */
	
    CANConfigureFilter      (CAN2, CAN_FILTER0, CAN2_MSG_ID, CAN_SID);
    CANConfigureFilterMask  (CAN2, CAN_FILTER_MASK0, SID_FILTER_MASK, CAN_SID,
                                CAN_FILTER_MASK_IDE_TYPE);
 ...
 * will have SID LED1_INDICATION_MSG and will be accepted by Channel 1 on CAN1.
...
/* Form an extended ID CAN message. Start by clearing the message buffer. */
    ...
    message->msgSID.SID = (WORD) (LED1_INDICATION_MSG & SID_BIT_MASK);
    message->msgEID.IDE = 0;		/* IDE = 1 means Extended ID message.*/
    message->msgEID.DLC = 1;		/* Send one byte of data.*/
    message->msgEID.RTR = 0;            /* For SID message Set RTR to 0 */
 ...
 * flush the channel when it receives an RTR message with SID 0x0201. */
}
 ...
 *                  SID CAN2_MSG_ID. This function is called every 10
...
/* Form a Standard ID RTR CAN message. Start by clearing the buffer. */
        ...
        message->msgSID.SID = (WORD) (CAN2_MSG_ID & SID_BIT_MASK);
        message->msgEID.IDE = 0;    /* IDE = 0 means standard ID message. */
        message->msgEID.RTR = 1;    /* For SID message Set RTR to 1 */
