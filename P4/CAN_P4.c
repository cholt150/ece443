...
*/
    CANConfigureChannelForRx(CAN1, CAN_CHANNEL1, CAN2_FIFO_BUFFERS,
                             CAN_RX_FULL_RECEIVE);
 ...
 * Example:        CAN1RxMsgProcess();
  ***************************************************************************/
void CAN1RxMsgProcess(int* IR_data, float* RPS, int* current_PWM)
{
 ...
 * function return value for null to check if a message has been received. */
    ...
    temp_IR = (message->data[0] | (message->data[1] << 8));
    
    temp_RPS = (message->data[2] | (message->data[3] << 8) | (message->data[4] << 16));
    
    temp_PWM = (message->data[5] | (message->data[6] << 8));
    
    *IR_data = temp_IR;
    *RPS = ((float)temp_RPS)/100.0;
 ...
 * Example:    CAN2RxMsgProcess();
  ***************************************************************************/
int CAN2RxMsgProcess(CANTxMessageBuffer * sensor_packet)
{
 ...
 * Description:
 *   This function will send a message to CAN2 with EID 0x8004002. The data 
 *   payload size is 1 byte. The value of the LEDAStatus will be toggled and
 ...
 * Example:  CAN2TxSendLEDMsg();
  ***************************************************************************/
void CAN1TxSendMsg(int RTR, int desired_PWM)
{
 ...
 * value is null. */
 ...
 /* Form a Extended ID CAN message. Start by clearing the buffer. */
    ...
    //      message->msgEID.EID	= 0x4004;   // Send message to CAN2.
            ...
            message->data[0] = desired_PWM;  // This is the payload.
        }
 ...
 *   it sends one byte which is a toggle state for LED2 and LEDB. FIFO0 is
 ...
 * Example:    CAN2TxSendLEDMsg();
  ***************************************************************************/
void CAN2TxSendMsg(CANTxMessageBuffer * message)
{
    if(message != NULL)
    {
		...
		message->msgEID.IDE = 1;            /* IDE = 1 means EID message. */
        message->msgEID.RTR = 0;            /* Not an RTR message. */
        message->msgEID.DLC = 7;            /* Send 7 byte of data. */
 ...
 * Remarks:         ISR cannot be called directly.
  ***************************************************************************/
void __ISR(_CAN_1_VECTOR, ipl2) CAN1InterruptHandler(void)
{
